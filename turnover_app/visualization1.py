...
def visualize_data(input_file, full_report_file, leavers_report_file, output_dir):
    """
    ...
    predicted_leavers = df_full['Predicted_Churnover'].sum()
    percentage_Churnover_risk = (predicted_leavers / total_employees) * 100
    ...
    plt.figure(figsize=(10, 6))
    sns.scatterplot(
        x='Estimated_Churnover_Cost', 
        y='Probability_Churnover', 
        hue='Predicted_Churnover', 
        data=df_full, 
        palette='viridis', 
        alpha=0.6,
    ...
    plt.title('2. Churnover Risk vs. Estimated Cost', fontsize=14)
    plt_max_cost = df_full['Estimated_Churnover_Cost'].max() * 1.05
    plt.xlim(0, plt_max_cost)
    plt.legend(title='Predicted Churnover', loc='lower right')
    path = os.path.join(output_dir, '2_cost_vs_probability.png')
    plt.savefig(path)
    plt.close()
    visualization_paths['cost_vs_probability'] = path

    # --- VISUALIZATION 3: Churnover Rate by Salary Level ---
    Churnover_rate = df_input.groupby('salary')['left'].mean().reset_index(name='Churnover_Rate')
    salary_order = ['low', 'medium', 'high']
    Churnover_rate['salary'] = pd.Categorical(Churnover_rate['salary'], categories=salary_order, ordered=True)
    Churnover_rate.sort_values('salary', ascending=True, inplace=True)
    
    plt.figure(figsize=(8, 5))
    # Using specific colors to ensure robustness against palette errors
    sns.barplot(x='salary', y='Churnover_Rate', data=Churnover_rate, palette=['#FF9999', '#99FF99', '#9999FF'], order=salary_order)
    plt.title('3. Employee Churnover Rate by Salary Level', fontsize=14)
    plt.ylim(0, Churnover_rate['Churnover_Rate'].max() * 1.1)
    path = os.path.join(output_dir, '3_Churnover_by_salary.png')
    plt.savefig(path)
    plt.close()
    visualization_paths['Churnover_by_salary'] = path
    ...
    # --- VISUALIZATION 5: Efficiency vs Predicted Churnover ---
    plt.figure(figsize=(8, 5))
    sns.boxplot(x='Predicted_Churnover', y='efficiency_score', data=df_full, palette='Set3')
    plt.title('5. Employee Efficiency Score vs Predicted Churnover', fontsize=14)
    plt.xticks([0, 1], ['0 - Predicted Stay', '1 - Predicted Leave'])
    path = os.path.join(output_dir, '5_efficiency_vs_predicted_Churnover.png')
    plt.savefig(path)
    plt.close()
    visualization_paths['efficiency_vs_Churnover'] = path
    ...
    final_output = {
        "metrics": {
            ...
            "Churnover_risk_percent": round(percentage_Churnover_risk, 2),
