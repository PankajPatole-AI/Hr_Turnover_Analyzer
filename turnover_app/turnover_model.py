...
MODEL_BASE_NAME = "Churnover_analysis"
MODEL_FILENAME = f"{MODEL_BASE_NAME}_log_reg_Churnover_model.pkl"
PREPROCESSOR_FILENAMES = {
    'SCALER': f"{MODEL_BASE_NAME}_scaler_X.pkl",
    'IMPUTER': f"{MODEL_BASE_NAME}_imputer.pkl"
}

def load_or_train_Churnover_model(X_train, X_test, y_train, features):
    """
    ...
    Returns:
        tuple: (log_reg_Churnover, imputer, scaler_X, X_test_scaled)
    """
    ...
    if os.path.exists(MODEL_FILENAME):
        with open(MODEL_FILENAME, 'rb') as f:
            log_reg_Churnover = pickle.load(f)
        log_reg_Churnover.set_params(warm_start=True, max_iter=1000)
    else:
        log_reg_Churnover = LogisticRegression(max_iter=1000, warm_start=True)

    # 3. Train/Update the Model
    log_reg_Churnover.fit(X_train_scaled, y_train)

    return log_reg_Churnover, imputer, scaler_X, X_test_scaled