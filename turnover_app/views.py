...
from .main_script import run_Churnover_analysis, run_visualization
...
def upload_file(request):
    ...
    if request.method == 'POST' and request.FILES.get('data_file'):
        ...
        try:
            ...
            leavers_report_path, full_list_report_path = run_Churnover_analysis(
...
# 3. Loop over the paths and create the MANY ResultImage objects
            ...
            if os.path.exists(temp_file_path):
                ...
                # The run_Churnover_analysis places the outputs in temp_dir as well
...
def dashboard_view(request, result_id):
    ...
    # 2. Get all visualization images
    visualizations_from_db = result.visualizations.all()
    
    # 3. --- NEW: Define the dashboard layout for 11 items ---
    # This layout places 2 small KPIs at the top and a 3x3 grid of 9 graphs below.
    dashboard_layout = [
    # --- Row 1: KPIs (larger + centered) ---
    {"x": 1, "y": 1, "w": 30, "h": 22},   # KPI 1
    {"x": 35, "y": 1, "w": 30, "h": 22},  # KPI 2

    # --- Row 2: Graphs (bigger width, reduced gaps) ---
    {"x": 1, "y": 26, "w": 32.33, "h": 28},  # Graph 1
    {"x": 34, "y": 26, "w": 32.33, "h": 28}, # Graph 2
    {"x": 67, "y": 26, "w": 32.33, "h": 28}, # Graph 3

    # --- Row 3 ---
    {"x": 1, "y": 56, "w": 32.33, "h": 28},  # Graph 4
    {"x": 34, "y": 56, "w": 32.33, "h": 28}, # Graph 5
    {"x": 67, "y": 56, "w": 32.33, "h": 28}, # Graph 6

    # --- Row 4 ---
    {"x": 1, "y": 86, "w": 32.33, "h": 28},  # Graph 7
    {"x": 34, "y": 86, "w": 32.33, "h": 28}, # Graph 8
    {"x": 67, "y": 86, "w": 32.33, "h": 28}, # Graph 9
]

    # 4. Combine visualizations with their layout
    visualizations_for_template = []
    
    for i, viz_obj in enumerate(visualizations_from_db):
        # Stop if we have more images than layout spots
        if i >= len(dashboard_layout):
            break
            
        visualizations_for_template.append({
            'image': viz_obj.image,
            'alt_text': viz_obj.alt_text,
            'layout': dashboard_layout[i] # Assigns the layout based on order
        })

    # 5. Send the new list to the template
    context = {
        'result': result,
        'visualizations': visualizations_for_template, 
    }
