...
def calculate_Churnover_cost_and_priority(df, employee_ID, log_reg_model, X_scaled_full, df_efficiency_scaled):
    """
    Calculates predicted Churnover probability, estimated cost, and retention priority score.

    Args:
        df (pd.DataFrame): The main DataFrame containing features and labels.
        employee_names (pd.Series): The series of employee names.
        log_reg_model (LogisticRegression): The trained Churnover model.
    ...
    # 1. Predictions
    # Calculate Probability_Churnover using the trained model
    df['Probability_Churnover'] = log_reg_model.predict_proba(X_scaled_full)[:, 1]

    THRESHOLD = 0.5
    df['Predicted_Churnover'] = (df['Probability_Churnover'] >= THRESHOLD).astype(int)
    ...
    # Calculate Estimated Churnover Cost
    df_full_output['Estimated_Churnover_Cost'] = (
    ...
    df_full_output['Retention_Priority_Score'] = (
        df_full_output['Probability_Churnover'] * df_full_output['Estimated_Churnover_Cost']
    )
    ...
    final_cols = ['Emp_ID', 'efficiency_score', 'Probability_Churnover', 'Predicted_Churnover',
                  'Estimated_Churnover_Cost', 'Retention_Priority_Score'] + \
                 [c for c in df_full_output.columns
                  if c not in ['Emp_ID', 'efficiency_score', 'Probability_Churnover', 'Predicted_Churnover',
                               'Estimated_Churnover_Cost', 'Retention_Priority_Score']]
